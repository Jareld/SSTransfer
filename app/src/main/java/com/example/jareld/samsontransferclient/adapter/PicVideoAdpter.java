        ...
        extends RecyclerView.Adapter<PicVideoAdpter.MyHolder>
{
    ...
    public PicVideoAdpter(Context context, ArrayList<File> picsVideos) {
        mContext = context;
    ...
    @Override
    public void onBindViewHolder(final PicVideoAdpter.MyHolder holder, final int position) {
        Glide.with(mContext)
        ...
        holder.mIv_main.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(holder.mIv_dagou.getVisibility() == View.GONE){
                    ...
                    mBooleens.set(position , true);
                }else{
                    //目前是出现的  就让他消失
                    holder.mIv_dagou.setVisibility(View.GONE);
                    mBooleens.set(position , false);
...
public ArrayList<String> getSelectedPV(){
    ...
    for(int i = 0 ; i < mBooleens.size() ; i ++){
            if(mBooleens.get(i)){
                arrayList.add(picsVideosPath.get(i).getAbsolutePath());
            }
    }
    return arrayList;
}
